...
holding(X, [pickUp(X, P) | S]):- not holding(X,S).
holding(X, [A|S]) :-
    not A = putDown(X,_),
    holding(X,S).

numHolding(Count, [pickUp(_, _) | S]) :-
    numHolding(Count2, S),
    Count is Count2 + 1.

numHolding(Count, [putDown(_, _) | S]) :-
    numHolding(Count2, S),
    Count is Count2 - 1.

numHolding(Count, [A | S]) :-
    not A = pickUp(_, _),
    not A = putDown(_, _),
    numHolding(Count, S).
...
faucetOn([turnOnFaucet | S]):- not faucetOn(S).
faucetOn([A|S]):-
    not A = turnOnFaucet,
    not A = turnOffFaucet,
    faucetOn(S).
...
% 1: useless using a scrubber without soap
useless(scrub(_, Y), [addSoap(Y) | S]) :- holding(Y, S), not soapy(Y, S).

% 2: useless adding soap to a scrubber that is already soapy
useless(addSoap(X), [soapy(X, _) | _]).

% 3: useless turning on the faucet if it was just turned off
useless(turnOnFaucet, [turnOffFaucet | _]).

% 4: useless turning off the faucet if it was just turned on
useless(turnOffFaucet, [turnOnFaucet | _]).

% 5: useless rinsing twice
useless(rinse(X), [rinse(X) | _]).

% 6: useless picking up an item if it was just put down
useless(pickUp(X, _), [putDown(X, _) | _]).

% 7: useless adding soap twice
useless(addSoap(X), [addSoap(X) | _]).

% 8: useless putting down an item if not holding any items
useless(putDown(_, _), [numHolding(0, _) | _]).

% 9: useless scrubbing an item twice
useless(scrub(X, _), [scrub(X, _) | _]).

% 10: useless scrubbing if the scrubber is not soapy
useless(scrub(_, Y), [not soapy(Y, _) | _]).

% 11: useless putting a dirty dish in the dish rack
useless(putDown(X,dish_rack), S) :- dirty(X,S).
